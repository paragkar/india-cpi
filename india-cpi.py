...
# Sort description options based on the weight in descending order
def extract_weight(description):
    match = re.findall(r'\((\d+\.\d+)\)', description)
    return float(match[0]) if match else 0.0

description_options = sorted(description_options, key=extract_weight, reverse=True)

# Multiselect dropdown
selected_description = st.sidebar.multiselect("Select Description to Display", description_options, default=description_options if selected_sector_type != "All" else [])
