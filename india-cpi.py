...
if selected_sector_type == "All" and not selected_description:
    ...
    slider_label = f"Slider for Selecting Date Index ({unique_dates[0].strftime('%b %Y')})"
    slider = slider_placeholder.slider(slider_label, min_value=0, max_value=len(unique_dates) - 1, value=0, key="date_slider")
    ...
    if play_button:
        for i in range(len(unique_dates)):
            ...
            slider_label = f"Slider for Selecting Date Index ({unique_dates[i].strftime('%b %Y')})"
            slider_placeholder.slider(slider_label, min_value=0, max_value=len(unique_dates) - 1, value=i, key=f"date_slider_play_{i}")
            time.sleep(0.5)  # Adjust the sleep time to control the animation speed
    else:
        selected_date = unique_dates[slider]
        update_plot(selected_date)
        update_title(selected_date)
        slider_label = f"Slider for Selecting Date Index ({selected_date.strftime('%b %Y')})"
        slider_placeholder.slider(slider_label, min_value=0, max_value=len(unique_dates) - 1, value=slider, key="date_slider")
